...
SrGeogebra is an ILIAS Plugin which is used to display GeoGebra (.ggb) files.
...
The user is now able to edit the advanced settings by changing the values of the inputs as desired and clicking the button "Save". The object has now been edited. The user may use the "< Page" button to return back to the edit page.
...
> Most advanced settings are listed in here: https://wiki.geogebra.org/en/Reference:GeoGebra_App_Parameters

##### Usage

A user is able to edit the default settings by accessing the plugin configuration under __Administration -> Plugins -> Actions/Configure__.


The user is now able to edit the default settings by changing the values of the inputs as desired and clicking the button "Save". The default settings have now been set.
